...
<main id="main" class="main">
  ...
  <div class="pagetitle">
    <h1>NOMBRES DES TACHES NON VALIDE : {{ $tachesNumber}}</h1>
    <nav></nav>
  </div><!-- End Page Title -->
  ...
  <section class="section">
    <div class="row">
      <div class="col-lg-12">
        ...
        <div class="card">
          <div class="card-body">
            <h5 class="card-title">TACHES PERSONNEL  {{$storedUser->id}}</h5>
              ...
              <tbody>
                @foreach($tachesAll as $taches)
                  <tr>
                    ...
                    <td>{{$taches->user_action}}</td>
                    <td>{{$taches->statut}}</td>
                    @if($taches->statut=="Non-Finish")
                    <td><a class="btn btn-danger" href="{{route('fin.taches',['id'=>$taches->id])}}"><i class="bi bi-bookmark-check"></i></a></td>
                    @else
                    <td><span style="color: blue;">En-attente-Confirmation</span></td>
                    @endif
                  </tr>
                @endforeach
              </tbody>
